...
const INSTANCE_IMAGES: [&str; 1] = [
    "Arch-Linux-x86_64-cloudimg.qcow2"
];

pub fn create_instance(instance: Instance) -> Result<(), Box<(dyn std::error::Error + Send + Sync + 'static)>> {
    let instance_type = &INSTANCE_TYPES[instance.itype as usize];

    std::fs::copy(format!("/etc/helio/images/{}", INSTANCE_IMAGES[instance.image as usize]), format!("/etc/helio/disks/{}.qcow2", instance.uuid))?;

    let output = common::create_process("qemu-img", vec![
		"resize",
...
pub fn delete_instance(instance: Instance) -> Result<(), Box<(dyn std::error::Error + Send + Sync + 'static)>> {
...
pub fn start_instance(instance: Instance) -> Result<(), Box<(dyn std::error::Error + Send + Sync + 'static)>> {
    match read_pid(format!("/etc/helio/pids/{}.pid", instance.uuid).as_str()) {
        Ok(pid) => {
            let s = System::new_all();
    ...
    let output = common::create_process(
        ...
        vec![
            ...
            "-device", format!("virtio-net-pci,netdev=net0,mac={}", instance.mac).as_str(),
